...
object Pipeline {
  ...
  /**
   * Dynamically create a chained pipeline task from a setting that contains the keys for the stages.
   */
  def chain[A](stagesKey: TaskKey[Seq[TaskKey[A => A]]]): Initialize[Task[A => A]] =
